...
export const PromptSelector: React.FC<PromptSelectorProps> = ({ options, onChange }) => {
  ...
  return (
    <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-4 shadow-xl backdrop-blur-sm">
        ...
        {items.map((item) => (
          ...
          >
            <div className="flex flex-col">
              <span className={`font-semibold text-xs ${options[item.key as keyof PromptOptions] ? 'text-banana-400' : 'text-slate-200'}`}>
                {item.label}
              </span>
